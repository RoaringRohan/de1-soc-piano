...
#include "defines.h"
#include "stopwatch.h"
#include "KEY.h"         // wrapper for /dev/IntelFPGAUP/KEY
#include "LEDR.h"        // wrapper for /dev/IntelFPGAUP/LEDR
...
typedef struct {
    ...
    int tone_idx;          // which tone (0 to 12)
...
// playback_start_ms is already declared above as a global long long
...
// Keyboard mapping: keycode mapped to tone index
...
// Audio helper: write one mixed sample
...
static void start_playback(void) {
    ...
    // Reset tones before playback (same behavior as before)
    pthread_mutex_lock(&mutex_tone_volume);
    ...
    // DO NOT call stopwatch_run() here; we control the display manually.
    led_play_on();
...
static void handle_playback_step(void) {
    ...
    // 1) Use our software timer for playback scheduling
    ...
    if (elapsed_cs <= playback_lead_cs) {
        ...
        // This makes KEY1 countdown start at time between record press and first note.
    ...
    // else: countdown is done AND no events left; stop_playback() will handle HEX off.

    // 3) Stop playback when all events consumed
    if (playback_index >= num_events) {
        // set playing = 0, turn off LEDR, call stopwatch_nodisplay(), release keys so audio fades out naturally
        stop_playback();
    }
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Part 5 running. Use piano keys (Q,2,W,3,E,R,5,T,6,Y,7,U,I).\n");
